...
export const createState = function (inParentComponent: React.Component) {
    return {
        layout: [] as number[][][],
        selectedTiles: [] as ISelectedTile[],
        score: { player: 0, opponent: 0 } as IScore,
        ...
        handleMessage_update: function (inPID: string, inScore: number) {
            if (inPID !== this.state.pid) {
                const scores: IScore = { ...this.state.scores };
        ...
        tileClick: function (inLayer: number, inRow: number, inColumn: number) {
            ...
            if (currentTileValue > 1000) {
                ...
                for (let i: number = 0; i < selectedTiles.length; i++) {
                    ...
                    if (selectedTile.layer == inLayer && selectedTile.row == inRow &&
            ...
            if (selectedTiles.length === 2) {
                if (selectedTiles[0].type === selectedTiles[1].type ||
                    ...
                    this.state.socketComm.send(`math_${this.state.pid}_${calculatedPoints}`);
